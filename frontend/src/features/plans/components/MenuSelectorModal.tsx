...
export const MenuSelectorModal: React.FC<MenuSelectorModalProps> = ({
  ...
  const filteredDishes = useMemo(() => {
    return catalog.filter((dish) => {
      ...
      // 3. Recomendaci├│n Inteligente (~ Meta cal├│rica +- 180 kcal o misma categor├¡a de comida)
      if (smartMatchOnly) {
        ...
        const isCategoryMatch =
          ...
          (mealConfig.name.toLowerCase().includes('colaci├│n') && (dish.category === 'Colaciones' || dish.category === 'Bebidas y Batidos'));
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
      <div className="bg-surface rounded-3xl border border-border p-6 md:p-8 max-w-4xl w-full shadow-2xl space-y-6 max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="flex items-start justify-between border-b border-border pb-4 shrink-0">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/20 text-primary text-xs font-black uppercase tracking-wider mb-1">
              <span>Asignaci├│n de Men├║ Cl├¡nico</span>
            </div>
            <h3 className="text-2xl font-extrabold text-foreground">
              {dayName} ΓÇó <span className="text-primary">{mealConfig.name}</span>
            </h3>
            <p className="text-muted text-xs mt-0.5 flex items-center gap-2 font-bold">
              ...
              <span>ΓÇó</span>
              <span className="text-emerald-600 dark:text-emerald-400">Meta Cal├│rica de la Toma: ~{targetCals} kcal</span>
            </p>
          </div>
        ...
        {/* Barra de B├║squeda y Bot├│n WOW Recomendador */}
        <div className="space-y-4 shrink-0">
          <div className="flex flex-col sm:flex-row items-center gap-3">
            <div className="relative flex-1 w-full">
              ...
              <input
                type="text"
                placeholder="Buscar por receta, ingrediente o etiqueta (ej. Prote├¡na, Avena, Salm├│n)..."
                value={searchTerm}
            ...
            {/* ≡ƒöÑ WOW Feature: Smart Recommendation Button */}
            <button
              type="button"
            ...
            >
              ...
              <span>Recomendaci├│n Inteligente (~{targetCals} kcal)</span>
            </button>
          </div>
          ...
          {smartMatchOnly && (
            <div className="bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 rounded-2xl p-3 flex items-center justify-between text-xs font-bold text-amber-800 dark:text-amber-300 animate-fade-in">
              <span className="flex items-center gap-2">
                ...
                <span>Mostrando platos ├│ptimos en calor├¡as (~{targetCals} kcal) y concordancia con {mealConfig.name}.</span>
              </span>
        ...
        {/* Lista / Cat├ílogo de Platos (Scrollable) */}
        <div className="overflow-y-auto flex-1 pr-1 space-y-3">
          {filteredDishes.length === 0 ? (
            <div className="text-center py-12 border-2 border-dashed border-border rounded-3xl space-y-3">
              <p className="text-muted font-bold text-base">No se encontraron platos que coincidan con tu b├║squeda.</p>
              {smartMatchOnly && (
                <button
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-muted mt-1 font-medium italic">
                        Porci├│n base: {dish.defaultPortion}
                      </p>
                    ...
                    <div className="mt-4 pt-3 border-t border-border/60 space-y-3">
                      ...
                      <div className="grid grid-cols-4 gap-1 text-center bg-surface-hover/60 p-2 rounded-xl border border-border/40">
                        <div>
                          <span className="block text-[10px] text-muted font-extrabold uppercase">Calor├¡as</span>
                        ...
                        <div>
                          <span className="block text-[10px] text-muted font-extrabold uppercase">Prote├¡na</span>
                      ...
                      {/* Ajustador de Porci├│n y Bot├│n A├▒adir */}
        ...
        {/* Pie del Modal con opci├│n para a├▒adir nota extra */}
        <div className="pt-4 border-t border-border shrink-0 flex flex-col sm:flex-row items-center justify-between gap-4">
          <input
            type="text"
            placeholder="Nota opcional para el paciente (ej. Acompa├▒ar con t├⌐ verde sin az├║car)..."
            value={notes}

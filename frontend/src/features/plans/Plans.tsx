...
export function Plans() {
  ...
  // Si se desactivan fines de semana y estamos en S├íbado/Domingo, mover a Lunes
  useEffect(() => {
    if (activePlan && !activePlan.includeWeekends) {
      if (selectedDay === 'S├íbado' || selectedDay === 'Domingo') {
  ...
  const handleToggleWeekends = () => {
    ...
    showToast(`Fines de semana ${nextVal ? 'activados (7 d├¡as)' : 'desactivados (5 d├¡as)'}.`);
  };
  ...
  const handleAddCustomMeal = () => {
    if (!activePlan) return;
    const customName = prompt('Nombre de la nueva comida o colaci├│n (ej. Pre-entrenamiento):', 'Colaci├│n extra');
  ...
  const handleCloneDayToAll = () => {
    ...
    // Clonar la estructura de comidas a los dem├ís d├¡as
    setPlans((prev) =>
      prev.map((p) => {
        if (p.id !== activePlan.id) return p;
        const clonedDays = p.days.map((dayObj) => {
          if (dayObj.day === selectedDay) return dayObj;
          // Copiar comidas con nuevos IDs ├║nicos pero mismos valores
    ...
    showToast(`ΓÜí Estructura del ${selectedDay} copiada con ├⌐xito a toda la semana.`);
  };
  ...
  const handleAssignDish = (dish: any, portionMultiplier: number, notes?: string) => {
    ...
    const newAssigned = {
      ...
      dishId: dish.id,
      name: dish.name,
      portion: portionMultiplier !== 1 ? `${portionMultiplier}x porci├│n (${dish.defaultPortion})` : dish.defaultPortion,
    ...
    showToast(`Γ£¿ Plato "${dish.name}" asignado a ${day} - ${meal.name}`);
  };

  const handleSaveConfiguration = () => {
    localStorage.setItem('dkfitt_plans', JSON.stringify(plans));
    showToast('≡ƒÆ╛ Estructura semanal del plan guardada correctamente en el sistema.');
  };
  ...
  const handleCreateNewPlan = (e: React.FormEvent) => {
    ...
    const newPlan: WeeklyPlan = {
      ...
      objective: newPlanObjective.trim() || 'Estructuraci├│n alimenticia equilibrada',
    ...
    setNewPlanPatient('');
    setNewPlanObjective('');
    showToast(`Nuevo plan "${newPlan.title}" creado con ├⌐xito.`);
  };
  ...
  return (
    <div className="space-y-8 pb-16">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-primary font-bold text-xs uppercase tracking-wider">
            <FileText size={16} />
            <span>Nutrici├│n ΓÇó Estructura Semanal</span>
          </div>
          <h1 className="text-3xl font-extrabold text-foreground tracking-tight mt-1">Planes Alimenticios</h1>
          <p className="text-muted text-sm mt-1">
            Define la distribuci├│n de comidas, horarios y objetivos cal├│ricos de lunes a viernes o fines de semana.
          </p>
        </div>
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2">
            ...
            <p className="text-gray-300 text-sm flex items-center gap-1.5 max-w-2xl">
              ...
              <span><strong className="text-white">Objetivo Cl├¡nico:</strong> {activePlan.objective}</span>
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 shrink-0 bg-white/5 p-4 rounded-2xl backdrop-blur-md border border-white/10">
            <div className="text-center sm:text-left px-2">
              <span className="text-[11px] font-bold text-gray-400 uppercase tracking-wider block">Promedio Cal├│rico Semanal</span>
              <div className="flex items-center justify-center sm:justify-start gap-1.5 text-amber-400 font-black text-2xl mt-0.5">
                ...
                <span>~{weeklyAverageCalories} kcal/d├¡a</span>
              </div>
            </div>
          ...
          <div className="bg-surface rounded-3xl border border-border p-6 md:p-8 max-w-md w-full shadow-2xl space-y-6">
            ...
            <form onSubmit={handleCreateNewPlan} className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  rows={2}
                  placeholder="ej. P├⌐rdida de peso controlada con alta saciedad"
      ...
      {/* Modal de Selecci├│n y Asignaci├│n de Platos */}
      {selectedSlotForMenu && (
        <MenuSelectorModal

...
export function FoodFormModal({ isOpen, onClose, onSave, initialData }: FoodFormModalProps) {
  ...
  // C├ílculo de calor├¡as en tiempo real: (P * 4) + (C * 4) + (G * 9)
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!servingSize.trim()) {
      setError('Por favor, especifica la raci├│n o unidad de medida.');
      return;
    }
    if (calories === '' || Number(calories) < 0) {
      setError('Las calor├¡as deben ser un n├║mero v├ílido mayor o igual a 0.');
      return;
    }
        ...
        <div className="bg-surface-hover/80 border-b border-border px-8 py-6 flex items-center justify-between">
          <div>
            <span className="text-xs font-extrabold uppercase tracking-widest text-primary block mb-1">
              {initialData ? 'Edici├│n de Registro' : 'Nuevo Registro Nutricional'}
            </span>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          ...
          <div>
            ...
            <span className="text-[11px] text-muted font-medium mt-1.5 block">
              ≡ƒÆí Todas las calor├¡as y macronutrientes deben ser calculados en base a esta raci├│n.
            </span>
          </div>
          ...
          {/* Calor├¡as y Calculadora en Tiempo Real */}
          <div className="space-y-4">
            <div>
              <label className="block text-xs font-extrabold uppercase tracking-wider text-muted mb-2 flex items-center justify-between">
                <span>Calor├¡as Totales (kcal) *</span>
              </label>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <span className="text-xs font-bold text-foreground block">
                    C├ílculo Estimado de Macros: <span className="font-extrabold text-sm">{estimatedCalories} kcal</span>
                  </span>
                  <span className="text-[11px] text-muted font-medium block">
                    {isDiffSignificant
                      ? 'Difiere un poco de las calor├¡as ingresadas. ┬┐Deseas aplicar el valor calculado?'
                      : 'C├ílculo est├índar basado en (P├ù4 + C├ù4 + G├ù9).'}
                  </span>
                </div>
              </div>
